...
export const FeatureAnalysisCard: React.FC<FeatureAnalysisCardProps> = ({ threat }) => {
  // Compute realistic feature comparisons derived from threat parameters
  const features = [
    {
    ...
    <Card className="p-6">
      <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-800">
        <h3 className="text-xs font-mono uppercase font-bold text-slate-200 tracking-wider flex items-center gap-2">
          <BarChart2 className="h-4 w-4 text-emerald-400" /> FEATURE BASELINE DEVIATION ANALYSIS
        </h3>
        ...
        {features.map((f, i) => (
          <div key={i} className="space-y-1.5 font-mono text-xs">
            <div className="flex items-center justify-between">
              ...
              <span className="text-orange-400 font-bold">{f.deviation} over baseline</span>
            </div>
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between text-[10px] text-slate-400">
                ...
                <span className="text-orange-400 font-bold">Anomalous</span>
              </div>
